...
class SVRG(Optimizer):
    ...
    def __init__(self, sgd_optimizer, loss_func, use_locking=False, name=None):
        ...
        if name is None:
            name = 'SVRG_with_' + sgd_optimizer.name
    ...
    def _create_slots(self, var_list):
        for v in var_list:
            with ops.colocate_with(v):
                ...
                v_prev = tf.Variable(v)
            self._get_or_make_slot(v, batch_gradient, "batch_gradient", self._name)
            self._get_or_make_slot(v, v_prev, "v_prev", self.name)
    ...
    def recompute_batch(self, batch_loss, var_list):
        ...
        batch_grad_updates = [state_ops.assign(self.get_slot(var, "batch_gradient"), grad)
                                for grad, var in batch_gradients_and_vars]
